...
$columns = [];
$stmtCols = $pdo->query("DESCRIBE phmarriage");
...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO phmarriage (" . implode(',', $columns) . ") VALUES ($placeholders)";
    $stmt = $pdo->prepare($sql);

    try {
        $stmt->execute(array_values($params));
        header("Location: quick-search-marriage.php?added=1");
        exit;
    } catch (Exception $e) {
...
<head>
    <meta charset="utf-8">
    <title>Add PHCRIS Marriage Record</title>
    ...
    <style>
        ...
        input[type="text"], input[type="date"], input[type="time"], input[type="number"], select, textarea { width:100%; padding:8px 10px; margin-top:6px; border:1px solid #ccd0d6; border-radius:6px; box-sizing:border-box; }
...
<body>
<div class="container">
    <h2>Add New PHCRIS Marriage Record</h2>
    <form method="post" autocomplete="off">
        <?php
        // split columns into two columns where possible
        ...
        foreach ($colChunks as $chunk) {
            ...
            foreach ($chunk as $col) {
                $type = 'text';
                if (strpos(strtolower($col), 'date') !== false) $type='date';
                if (strpos(strtolower($col), 'time') !== false) $type='time';
                if (strpos(strtolower($col), 'number') !== false || strpos(strtolower($col), 'age') !== false) $type='number';

                // use textarea for long text fields
                $textareaFields = ['PlaceOfMarriage','MarriageLicensePlaceIssued','SolemnizingOfficerAddress','DelayedOrCorrected','Witness1Name','Witness2Name','Witness3Name','Witness4Name','SectionD','SectionE','MarriagePlaceRepeat','MarriageLicenseMarker','MarriageLicensePlaceIssuedRepeat','ReasonForDelayedRegistration','AffiantAddress','LocationSigned','MarriageOf','MarriagePlace','MarriageHusbandName','MarriageWifeName','AdminOfficerAddress','SolemnizingOfficerAddressRepeat'];
        ...
        <a href="quick-search-marriage.php" style="margin-left:12px;">Cancel</a>
    </form>
</div>
</body>
</html>
